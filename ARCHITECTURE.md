...
`singleRowTabs` preference) skips that measurement entirely and clears any breaks left from wrap
mode, and its observer is rebuilt on the toggle -- switching back mounts a row element the
original observer never saw. Its scrollbar is styled through `::-webkit-scrollbar` alone: the

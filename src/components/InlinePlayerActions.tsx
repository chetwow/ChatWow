...
export function InlinePlayerActions({ name, onOpenBrowser, onClose }: {
  name: string; onOpenBrowser: () => void; onClose: () => void;
}) {
  ...
  return (
    <span className="flex shrink-0 items-center gap-1">
      ...
      <button type="button" className={buttonClass} onClick={onClose} aria-label={`Close ${name} player`} title="Close player">

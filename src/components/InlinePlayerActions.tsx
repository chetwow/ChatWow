...
export function InlinePlayerActions({ name, onOpenBrowser, onClose, closeLabel }: {
  ...
  return (
    <span className="flex shrink-0 items-center gap-1">
      <button type="button" className={buttonClass} onClick={onOpenBrowser} aria-label={`Open ${name} in browser`} data-tooltip="Open in browser">
      ...
      <button type="button" className={buttonClass} onClick={onClose} aria-label={closeLabel ?? `Close ${name} player`} data-tooltip={closeLabel ?? "Close player"}>
